...
func (s *Service) GetCredentials(userID int64, provider string) (*UserCredentials, error) {
	...
	err := s.db.QueryRow(query, userID, provider).Scan(
		&creds.ID, &creds.UserID, &creds.Provider,
		&creds.AccessToken, &refreshToken,
		&creds.TokenType, &expiresAt, &scopes,
...
func (s *Service) SaveCredentials(creds *UserCredentials) error {
	query := `
		INSERT INTO user_credentials (user_id, provider, access_token, refresh_token, token_type, expires_at, scopes, updated_at)
	...
	err := s.db.QueryRow(
		query,
		creds.UserID, creds.Provider,
		creds.AccessToken, refreshToken,
		creds.TokenType, expiresAt, creds.Scopes,
	).Scan(&creds.ID)
...
// ToMap converts credentials to a map for sending to Vtoroy
