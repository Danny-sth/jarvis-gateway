...
// AudioMetadata represents audio file metadata from message_audio table
...
func (s *Service) GetRecentMessages(conversationID string, limit int) ([]Message, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&msg.ID, &msg.Role, &msg.Content, &toolCalls, &msg.CreatedAt); err != nil {
...
// GetMessagesAudio returns audio metadata for messages in a conversation
func (s *Service) GetMessagesAudio(conversationID string) (map[int64]AudioMetadata, error) {
	query := `
		SELECT ma.message_id, ma.duration_ms, ma.waveform
		FROM message_audio ma
	...
	if err != nil {
		return nil, fmt.Errorf("failed to query message audio: %w", err)
	}
	defer rows.Close()

	audioMap := make(map[int64]AudioMetadata)
	for rows.Next() {
		var meta AudioMetadata
		var waveformJSON sql.NullString
		if err := rows.Scan(&meta.MessageID, &meta.DurationMs, &waveformJSON); err != nil {
			return nil, fmt.Errorf("failed to scan audio metadata: %w", err)
		}
		if waveformJSON.Valid {
			if err := json.Unmarshal([]byte(waveformJSON.String), &meta.Waveform); err != nil {
				// Ignore waveform parse errors, leave as nil
			}
		}
		audioMap[meta.MessageID] = meta
	}

	return audioMap, nil
}

// GetMessageAudio returns audio data for a specific message
func (s *Service) GetMessageAudio(messageID int64) ([]byte, *AudioMetadata, error) {
	query := `
		SELECT audio_data, duration_ms, waveform
		FROM message_audio
		WHERE message_id = $1
	`
	...
	err := s.db.QueryRow(query, messageID).Scan(&audioData, &meta.DurationMs, &waveformJSON)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, nil, fmt.Errorf("audio not found for message %d", messageID)
		}
		return nil, nil, fmt.Errorf("failed to query audio: %w", err)
	}

	meta.MessageID = messageID
	if waveformJSON.Valid {
		if err := json.Unmarshal([]byte(waveformJSON.String), &meta.Waveform); err != nil {
			// Ignore waveform parse errors
		}
	}

	return audioData, &meta, nil
}

// SaveMessageAudio saves audio data for a message
func (s *Service) SaveMessageAudio(messageID int64, audioData []byte, durationMs int, waveform []float64) error {
	var waveformJSON interface{}
	if len(waveform) > 0 {
		data, err := json.Marshal(waveform)
		if err == nil {
			waveformJSON = string(data)
		}
	}

	query := `
		INSERT INTO message_audio (message_id, audio_data, duration_ms, waveform)
		VALUES ($1, $2, $3, $4)
	...
	_, err := s.db.Exec(query, messageID, audioData, durationMs, waveformJSON)
	if err != nil {
		return fmt.Errorf("failed to save message audio: %w", err)
	}
